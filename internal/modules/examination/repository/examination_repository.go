...
func (r *examinationRepository) GetSchedules(ctx context.Context, examID, periodID int) ([]*entity.JadwalUjian, error) {
	query := `
		SELECT j.idjadwalujian, j.idujian, u.namaujian, 
		       ...
		       COALESCE(nullif(trim(r.waktumulai), ''), nullif(trim(j.waktumulai), ''), '08:00') as jammulai, 
		       COALESCE(nullif(trim(r.waktuselesai), ''), nullif(trim(j.waktuselesai), ''), '09:30') as jamselesai, 
